...
export const generateCopywriting = (property: Property, postType: PostType, hookType: HookType): string => {
  const { 
    subarea, property_type, price_wan, build_ping, land_ping, parking, address_note,
  ...
  // 自然語意化的核心資訊 (符合 GEO 搜尋引擎偏好，加入 E-E-A-T 元素與長尾關鍵字)
  const baseInfo = `📍 【福哥實地走訪評估：物件核心資訊】
這是一間位於${subarea}的優質${property_type}。
在地深耕 30 年的福哥親自為您把關，這間總建坪達 ${build_ping} 坪，
擁有 ${layoutStr} 的完美格局，且${parking ? `附有${extractText(parking)}` : '目前無車位'}。
現在屋主開價僅需 ${price_wan} 萬，是${subarea}買屋首選的絕佳標的！`;

  const formatMustSay = (text: string) => {
    if (!text) return '';
    const points = text.split('\n').filter(Boolean);
    return points.map((p, i) => `✓ ${extractText(p)}`).join('\n');
  };

  // 整理必講特色為陣列
  const mustSayItems = formatMustSay(must_say_3);
  const mustSayStr = mustSayItems ? `🌟 【花蓮房產顧問福哥推薦：三大核心優勢】\n${mustSayItems}` : '';

  const contactInfo = `
👇 懂價值，就現在聯絡我：
📞 專線：0925-997779
💬 官方 LINE：https://lin.ee/uCoFu3E
👤 花蓮房產顧問福哥 周福良 (91登字010851號)
`;
...
`;
  ...
  const getHookText = () => {
    if (hookType === '無') {
      // 根據不同的 postType 給予專屬的無選項放大重點開場 (多種隨機替換，避免廣告疲乏)
      const randomHook = (hooks: string[]) => hooks[Math.floor(Math.random() * hooks.length)];
      ...
      switch (postType) {
        ...
        case '物件開箱':
        default:
          return randomHook(fallbackHooks['物件開箱']).replace('{subarea}', subarea).replace('{property_type}', property_type);
      }
    }
...
${title} 絕美釋出！${status_push === '強推' ? '🔥 福哥強力推薦，本週必看精選！\n' : ''}
...
${footer}`;
    ...
    case '知識教學':
      return `${hookPrefix}【📚 福哥房產小學堂：看懂 ${subarea} ${property_type} 的隱藏潛力】
很多客戶問我，為什麼這個區域的物件這麼搶手？
今天用我們這間「${title}」為例，帶你看懂市場優勢：
...
我是福哥。我不只是賣房子，我是幫你的資產把關。
專業，是我的標配；看準價值，才是我的本事。
