...
const mockAreas: Area[] = [
  { area_code: 'HC', area_name: '花蓮市', active: true },
  { area_code: 'JA', area_name: '吉安鄉', active: true },
  { area_code: 'XC', area_name: '新城鄉', active: true },
  { area_code: 'FL', area_name: '鳳林鎮', active: true },
  { area_code: 'YL', area_name: '玉里鎮', active: true },
  { area_code: 'GP', area_name: '瑞穗/玉里 | Ruisui/Yuli', active: false },
  { area_code: 'SF', area_name: '壽豐鄉', active: true },
  { area_code: 'KF', area_name: '光復鄉', active: true },
  { area_code: 'RS', area_name: '瑞穗鄉', active: true },
  { area_code: 'FR', area_name: '富里鄉', active: true },
  { area_code: 'OT', area_name: '其他', active: true },
  { area_code: 'SL', area_name: '秀林鄉', active: true },
];
...
const mockSubareas: Subarea[] = [
  ...
  { area_code: 'JA', subarea: '北昌/太昌', active: true, admin_district: '吉安鄉', strategy: '賣機能、賣便利' },
...
const mockClient: Client = {
  client_id: 'A0001',
  display_name: '福哥',
  login_token: 'A0001_20260416_K9x7p3m2Q5v8n1t6R4',
  plan_name: 'basic',
  monthly_quota: 300,
  month_key: '202604',
  used_this_month: 0,
...
// 預設選項資料
const mockMainPoints: OptionItem[] = [
  { value: '低於實價登 | Below market' },
...
const mockTargetBuyers: OptionItem[] = [
...
const mockPropertyTypes: OptionItem[] = [
  { value: '電梯大樓 / 華廈' },
  { value: '步梯公寓' },
  { value: '透天厝 (住宅)' },
  { value: '透天厝 (店住)' },
...
const mockParkingOptions: OptionItem[] = [
  { value: '有車位 | With parking' },
  { value: '無車位 | No parking' },
  { value: '可租車位 | Parking rentable' },
  { value: '可另購車位 | Parking for sale' },
];

const mockStatusNowOptions: OptionItem[] = [
  { value: '新進案' },
  { value: '銷售中' },
  { value: '議價中' },
  { value: '已成交' },
...
const mockStatusPushOptions: OptionItem[] = [
  { value: '待場勘拍照' },
  { value: '文案排版中' },
  { value: '全網熱銷中' },
  { value: '追蹤議價回報' },
...
const mockRoomOptions: OptionItem[] = [
  { value: '0' }, { value: '1' }, { value: '2' }, { value: '3' }, 
  { value: '4' }, { value: '5' }, { value: '6+' }
];

const mockHallOptions: OptionItem[] = [
  { value: '0' }, { value: '1' }, { value: '2' }, { value: '3' }, 
  { value: '4' }, { value: '5' }, { value: '6' }
];

const mockBathOptions: OptionItem[] = [
  { value: '0' }, { value: '1' }, { value: '2' }, { value: '3' }, 
  { value: '4' }, { value: '5' }, { value: '6' }
];

const mockBalconyOptions: OptionItem[] = [
  { value: '0' }, { value: '1' }, { value: '2' }, { value: '3' }, 
  { value: '4' }, { value: '5' }, { value: '6' }
];
...
export const useSystemStore = create<SystemState>()(
  persist(
    (set, get) => ({
      ...
      targetBuyers: mockTargetBuyers,
      propertyTypes: mockPropertyTypes,
      parkingOptions: mockParkingOptions,
      statusNowOptions: mockStatusNowOptions,
      statusPushOptions: mockStatusPushOptions,
      roomOptions: mockRoomOptions,
      hallOptions: mockHallOptions,
      bathOptions: mockBathOptions,
      balconyOptions: mockBalconyOptions,
      
      setCurrentClient: (client) => set({ currentClient: client }),
      addCopy: (copy) => set((state) => ({ copies: [copy, ...state.copies] })),
      markCopyAsUsed: (copy_id) => set((state) => ({
        copies: state.copies.map(c => c.copy_id === copy_id ? { ...c, used: true } : c)
      })),
      getNextListingId: (clientId, type, areaCode) => {
        ...
        if (type === 'C') {
          if (!areaCode) return '';
          key = `LISTING_${clientId}_${areaCode}`; // 加入 clientId 以區分不同仲介的流水號
          prefix = `C${areaCode}`;
        } else {
          key = `BUYER_${clientId}_${dateStr}`; // 加入 clientId 以區分不同仲介的流水號
          prefix = `B${dateStr}-`;
        }
        ...
        if (type === 'C') {
          return `${prefix}${String(nextValue).padStart(4, '0')}`; // CJA0001
        } else {
          return `${prefix}${String(nextValue).padStart(2, '0')}`; // B20260419-01
        }
      },
    }),
    {
      name: 'system-storage-v2',
    }
  )
);
