...
【HOOK 開場段生成規則】
- 產出一段文字，不換行，整段合計約35字（嚴格控制，絕對不超過40字）
- ${hookType === '無' ? '【HOOK 風格：不指定】由AI自行綜合主賣點、次賣點與物件整體特性，判斷最強的開場表達方式，主賣點方向永遠優先' : `依照 HOOK 風格（${hookType}）定調語氣，但【主賣點方向永遠優先】，HOOK 風格只影響表達方式，不能蓋掉主賣點的核心訊息`}
- 【主賣點方向對照】：
  · 主賣點含「急售、可談、降價、限時」等→ HOOK 必須帶出緊迫感和稀有機會，不能用與急售無關的家庭溫情場景開場
  · 主賣點含「景觀、採光、格局、空間」等→ 可用生活畫面和情感溫度
  · 主賣點含「投報、收租、商機、地段」等→ 聚焦財務邏輯和商業價值
  · 主賣點含「稀有、釋出、難得、機會」等→ 製造稀缺感和行動urgency
- HOOK 開場必須讓讀者感受到主賣點的核心張力，氣質選一種：有溫度、有專業、有故事、高HOOK衝擊、或高情緒價值
- 如有「AI 參考備註」內容，可融入增強說服力
- 商用物件：聚焦地段優勢、商業客流、商機、投報，禁止住宅語彙
- 土地物件：聚焦開發、增值、稀有性、地形優勢

【🎯 適合對象生成規則】
- 用一句平易近人的話描述「什麼樣的人適合買這個物件」，12字以內
- 必須說清楚是「什麼身份的人」，如：想自營的老闆、尋求收租的投資人、計畫換屋的家庭、首購預算有限的年輕人、想長期持有土地增值的買家
- 禁止使用「人流」「卡位」「動線」等行銷術語作為主詞，這些是賣點描述詞，不是對象描述
- 禁止把賣點當對象（例如「卡位主幹道人流」是在描述地點，不是在說誰適合買）
- ✅ 正確：「想自營或出租的投資客」「預算有限的首購族」「有創業夢的年輕老闆」
- ❌ 錯誤：「卡位主幹道人流」「地段增值投資者」（太模糊，不說清楚是什麼人）
...
▌押韻規則（自然為原則，不強制）：
- 押韻是加分選項，不是強制要求。若三行句尾能自然形成韻腳（如「財、來、快」同押-ai），則使用押韻
- 若無法自然押韻，寧可不押韻，直接寫出語意通順、語氣有力的金句
- ❌ 嚴格禁止：為了押韻而寫出語意不通、生硬、難懂的詞語（如「住得踏實賴」「福邸選得快賴」等意思不清的句子）
- ❌ 禁止：三行句尾用同一個漢字（如「健、健、健」）
- 底線原則：句子讀起來自然、有意義，遠比押韻更重要
