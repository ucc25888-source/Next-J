...
function dbRowToProperty(row: Record<string, unknown>): Property {
  return {
    ...
    img5_url: (row.img5_url as string) ?? '',
...
export async function POST(req: NextRequest) {
  ...
  await queryOne(
    `INSERT INTO properties (
      ...
      fb_post_count, img1_url, img2_url, img3_url, img4_url, img5_url,
      garden_area,
    ...
    [
      ...
      0,
      body.img1_url ?? '', body.img2_url ?? '', body.img3_url ?? '', body.img4_url ?? '', body.img5_url ?? '',
      body.garden_area ?? '',
