...
export async function POST(req: NextRequest) {
  ...
  await queryOne(
    `INSERT INTO properties (
      ...
      property_type, price_wan, build_ping, land_ping, rooms, halls, baths,
      balconies, parking, status_now, status_push, main_point, second_point,
      target_buyer, must_say_3, notes_private, img1_url, img2_url, img3_url, img4_url,
    ...
    [
      ...
      body.property_type ?? '', body.price_wan ?? 0, body.build_ping ?? 0,
      body.land_ping ?? 0, body.rooms ?? '0', body.halls ?? '0', body.baths ?? '0',
      body.balconies ?? '0', body.parking ?? '無車位', body.status_now ?? '新進案',
      body.status_push ?? '待場勘拍照', body.main_point ?? '', body.second_point ?? '',
