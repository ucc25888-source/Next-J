  ...
  return (
    <aside className="w-[220px] flex flex-col bg-titanium-950 border-r border-glacier-200/[0.07] min-h-screen shrink-0">
      ...
      <div className="px-5 pt-5 pb-2">
        <span className="text-[9px] font-bold text-glacier-500 tracking-[0.18em] uppercase">
          Navigation
        </span>
      </div>
            ...
            >
              ...
              <Icon
                className={`w-[15px] h-[15px] shrink-0 ${
                  isActive
                    ? "text-aurora-500"
                    : "text-glacier-500 group-hover:text-glacier-300"
                }`}
              />
              <span className="flex-1">{item.label}</span>
              {isActive && (
                <ChevronRight className="w-3 h-3 text-aurora-500/60" />
              )}
            </Link>
          );
        })}
      ...
      <div className="px-4 py-4 border-t border-glacier-200/[0.07]">
        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-titanium-900 border border-glacier-200/[0.06]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-semibold text-glacier-200 truncate">
              系統運行中
            </p>
